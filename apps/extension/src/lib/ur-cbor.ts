...
/**
 * Byte-string detection must not depend on `instanceof Buffer`: the browser
 * bundle can contain several Buffer polyfill copies, and cbor-sync's
 * `instanceof` check is what corrupted requestId/signData/address. borc
 * detects byte strings the same way (via the object's class tag), so any
 * Buffer/Uint8Array is encoded as a CBOR byte string.
 */
